...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event {
  ...
  if ( _mode == page_mode_curl_start ) {
    ...
    NSLog(@"ratio %f", _curl_ratio * CURL_BOOST);
...
- (CGRect) getAspectFittingImageRect:(UIImage *)im0 {
  ...
  if ( (image_width / image_height) > (im.size.width / im.size.height) ) {
    NSLog(@"original aspect %f",(im.size.width / im.size.height));
    NSLog(@"aspect %f",(image_width / image_height));
    image_margin_y = 0;
